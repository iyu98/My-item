  ...
  <div class="coming">
    <div class="film-list-wrapper wrapper" ref="film-wrapper">
      <ul class="film-list content">
        <li class="items"
          v-for="(item, index) in comingMovieList"
          :key="index"
        ...
        >
          <a href="javascript:void(0)" class="now-playing-film">
            ...
            <div class="film-info">
              ...
              <div class="film-grade info-col" v-if="grade">
                <span class="label">观众评分</span>
                <span class="grade">暂无评分</span>
              </div>
              <div class="film-grade info-col" v-else>
                &nbsp;
              </div>
              <div class="film-actor info-col">
                <span class="label">主演</span>
                <span class="label"
                  v-for="(aItem, aIndex) in item.actors"
                  :key="aIndex"
                >&nbsp;{{ aItem.name }}</span>
              </div>
...
import { getMovieComing } from 'api/api.js'
export default {
  data () {
    return {
      grade: false,
  ...
  created () {
    getMovieComing ()
      .then((res) => {
        // console.log(res)
        this.comingMovieList = res.films
      })
  },
  mounted () {
    this.initBs()
